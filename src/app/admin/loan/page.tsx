...
function LoanPage() {
  ...
  const generateDummyLoans = (count: number): Loan[] => {
    ...
    const statuses: ('Pending' | 'Approved' | 'Rejected' | 'Disbursed')[] = ['Pending', 'Approved', 'Rejected', 'Disbursed'];
    ...
    return Array.from({ length: count }, (_, index) => {
      ...
      return {
        ...
        interestRate: Math.random() * 5 + 3.5
      };
    });
  };
  ...
  const fetchLoanList = async () => {
    try {
      ...
      // Generate dummy data
      const dummyLoans = generateDummyLoans(25);
      setLoanList(dummyLoans);
      setFilteredLoans(dummyLoans);
    } catch (error) {
        ...
        <div className="grid grid-cols-1 md:grid-cols-5 gap-4 mb-8">
          {Object.entries(statusCounts).map(([status, count]) => (
            <div 
              ...
              className={`bg-gray-800 rounded-2xl p-4 border cursor-pointer transition-all duration-200 ${
          ...
          <div className="flex justify-between items-center">
            ...
            <div className="text-right">
              ...
              <p className="text-xl font-semibold text-white">
                {formatCurrency(filteredLoans.length > 0 ? 
                  filteredLoans.reduce((sum, loan) => sum + loan.amount, 0) / filteredLoans.length : 0
                )}
              </p>
            </div>
          </div>
